...
public class Hd2csv {
    ...
    public void run() throws IOException {
        ...
        for (Query query : queries) {
            ...
            if (json != null) {
                ...
                List<HDEntry> entries = new GsonBuilder().registerTypeAdapter(DateTime.class, new DateTimeDeserializer()).create().fromJson(json, listType);
                for (HDEntry entry : entries) {
                    ...
                    if (!result.containsKey(entry.timestamp.toInstant().getMillis())) {
                        result.put(entry.timestamp.toInstant().getMillis(), new HashMap<String, Double>(options.items.size()));
                    }
                    result.get(entry.timestamp.toInstant().getMillis()).put(query.item.intern(), entry.quality >= options.quality ? entry.value : null);
                }
            }
        }
    ...
    private String doQuery(Query query) {
        try {
            if (options.verbose) {
                System.err.println(String.format("executing request to %s with query parameters from=%s, to=%s, no=%s", query.uri,
                        query.queryParams.get("from"), query.queryParams.get("to"), query.queryParams.get("no")));
            }
            HttpResponse<String> jsonResponse = Unirest.get(query.uri.toString()).header("accept", "application/json")
                    ...
                    .queryString("to", query.queryParams.get("to")).queryString("no", query.queryParams.get("no")).asString();
    ...
    private List<Query> makeQueries() {
        ...
        for (String item : options.items) {
            ...
            if (options.url != null) {
                ...
                uri = URI.create("http://" + options.host + ":" + options.port + "/org.eclipse.scada.hd/items/" + item + "/" + options.aggregation + "/");
            }
